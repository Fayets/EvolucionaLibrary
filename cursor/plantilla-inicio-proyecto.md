...
- `main.py` — punto de entrada FastAPI (o el framework acordado), registro de routers, CORS, lifespan con **`init_db()`** (import de `src.models` + `db.generate_mapping` al arrancar).
...
- `src/db.py` — instancia `Database` de Pony y `db.bind(...)` con `DB_PROVIDER`, `DB_USER`, `DB_PASS`, `DB_HOST`, `DB_NAME` desde `.env.template` (`python-decouple`). **Solo conexión**; sin `generate_mapping` ni imports de entidades.
