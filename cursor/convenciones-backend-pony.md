...
- Los modelos (entidades) se definen con la API de Pony (`Database`, entidades con relaciones, etc.). `src/db.py` solo instancia `Database` y hace `db.bind(provider=..., user=..., password=..., host=..., database=...)` con las variables de `.env.template`; las entidades viven en `src/models.py` y el mapping se genera en el lifespan de `main.py` (`init_db`), según `cursor/plantilla-inicio-proyecto.md`.
...
- `main.py`: montar la app, incluir routers desde `src.controllers`, middleware mínimo, lifespan con `init_db()` (import de entidades + `generate_mapping`).
